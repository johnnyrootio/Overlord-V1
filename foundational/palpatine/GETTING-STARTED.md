...
- **multiclaude** installed and in PATH:
  ```bash
  go install github.com/dlorenc/multiclaude/cmd/multiclaude@latest
  # Add to PATH if needed: echo 'export PATH="$HOME/go/bin:$PATH"' >> ~/.zshrc
  # Verify: multiclaude version
  ```
  **Location**: `~/go/bin/multiclaude` (use full path if not in PATH)
...
# Note: Use 'multiclaude' if in PATH, or '~/go/bin/multiclaude' if not
multiclaude repo init https://github.com/your-org/your-repo
# Or use short alias if available: mc repo init https://github.com/your-org/your-repo
