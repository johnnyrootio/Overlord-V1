    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "require",
            "pattern": "gh pr create|gh pr open",
            "precondition": "./scripts/check.sh",
            "message": "Must run ./scripts/check.sh before creating PR"
          }
        ]
      }
    ]
...
1. **Invoke Claude Code with brainstorming superpower**:
   ```bash
   # In the repository directory, use Claude Code CLI with superpowers
...
2. **Use Context7 MCP** to gather context:
...
3. **Create detailed implementation plan**:
   ...
   Based on our brainstorming, create a detailed implementation plan for Phase 0 bootstrap:
...
4. **Review and refine the plan**:
   - Overlord reviews the plan for completeness
   - Identifies any missing components
   - Ensures alignment with workflow requirements
   - Refines based on project-specific needs
...
1. **Initialize the repository** (if not already done):
   ...
   **Note**: Repository creation requires GitHub CLI (`gh`) to be authenticated. If creation fails, the Overlord should ask the user to create it manually and provide the URL.

2. **Implement tech stack configuration** (per plan):
   - Create package manager configuration (package.json, requirements.txt, Cargo.toml, go.mod, etc.)
   - Set up build system
   - Configure development dependencies
   - Create minimal project structure (per plan)
...
7. **Create multiclaude agent overrides** (`.multiclaude/agents/`):
   - Use **ADDITIVE mode** (recommended) to augment default prompts
   - **How it works**: multiclaude appends these as "Repository-specific instructions" after defaults
   - **Why augmenting**: Works alongside defaults, but takes precedence when conflicts arise
   - **Benefit**: As multiclaude's defaults evolve, you get improvements automatically (when they don't conflict)
   - **Judgment**: If our prompts are better for spec-first enforcement, they take precedence
   - See [AGENT-PROMPTS](./AGENT-PROMPTS/) directory for augmenting override templates:
     - `worker.md` - Adds spec-first implementation guidance (authoritative when it conflicts)
     - `supervisor.md` - Adds test arbitration protocol (our process)
     - `reviewer.md` - Adds spec compliance verification (takes precedence)
   - These augmenting overrides enforce spec-first development and can override defaults when better

8. **Create hooks.json** (`.multiclaude/hooks.json`):
   - Block destructive commands
   - Require `./scripts/check.sh` before PR creation
   - Prevent CI weakening

9. **Create auto-accept script** (if unattended mode):
   - `scripts/auto_accept_workers.sh` to handle permission prompts
...
Repeat for each wave:
  ...
  4) If unattended mode: run auto-accept script
  5) Wait until:
