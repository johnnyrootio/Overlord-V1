...
## Evolution note: Reliably capturing from multiclaude status stream

**Observed (Robotic Barista Overlord):** The Overlord sometimes has trouble reliably capturing messages from the multiclaude status stream when using this flow (send → agents reply to workspace → Overlord runs `list-workspace-replies.sh`). Parsing or timing can make it hard for the Overlord to consistently see and act on replies.

**Tracked for evolution:**
- Improve reliability of Overlord capturing workspace replies (e.g. polling, clearer output format, or integration with Overlord tooling).
- Consider whether multiclaude or the Overlord workflow should expose a more stream-friendly or machine-readable way to consume agent replies.
- Document any workarounds that improve reliability (e.g. run script at fixed intervals, ack-after-read to avoid duplicates).

See also: `scripts/list-workspace-replies.sh`, and project-specific Overlord iterations (e.g. Robotic Barista) for current workarounds.
