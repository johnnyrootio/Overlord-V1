...
**You have access to several MCP tools. Use them to monitor and improve the workflow:**

1. **Reflection MCP** (CRITICAL):
   - **Monitor patterns**: `get_common_lessons(task="all")` to identify recurring issues
   - **After test arbitration**: `store_episode(task="test_arbitration", outcome="resolved", lessons=[...])`
   - **After deadlock resolution**: `store_episode(task="deadlock_resolution", outcome="resolved", lessons=[...])`
   - **Weekly review**: Review common lessons to identify patterns

2. **Context7 MCP**:
   - Research testing best practices when arbitrating tests
   - Look up patterns for test quality

3. **Probe MCP**:
   - Analyze codebase when investigating test failures
   - Understand dependencies in test arbitration

**Example workflow**:
```python
# When test arbitration needed
episodes = retrieve_episodes(task="test_arbitration")
# Review past arbitration decisions...

# After resolution
store_episode(
    task="test_arbitration",
    context={"issue": issue_number, "decision": decision},
    outcome="resolved",
    lessons=["What was decided", "Why"]
)
```
