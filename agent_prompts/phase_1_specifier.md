...
- **docs/testing-strategy.md** — Four layers, wave mapping, black box CRITICAL, test ticket patterns, definition of done, test arbitration. Definition of done includes **`./scripts/check.sh` passes** (per ECOSYSTEM-RULES)—working software is defined by that gate. Execution is **agent-driven**: agents (workers, CI) run check.sh; the human does not run it manually.
